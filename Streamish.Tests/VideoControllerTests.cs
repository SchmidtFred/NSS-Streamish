...
namespace Streamish.Tests
{
    public class VideoControllerTests
    {
        ...
        [Fact]
        public void Search_Method_Finds_Correct_Videos()
        {
            // Arrange
            var desiredResultCount = 4;
            var testString = "Cory Gochanour";
            var videosTitleTest = CreateTestVideos(10);
            var videosDescriptionTest = CreateTestVideos(10);
            for (var i = 0; i < desiredResultCount; i++) // Make sure we know the Titles and Descriptions of the videos
            {
                videosTitleTest[i].Title = testString;
                videosDescriptionTest[i].Description = testString;
            }

            var repoTitle = new InMemoryVideoRepository(videosTitleTest);
            var repoDescription = new InMemoryVideoRepository(videosDescriptionTest);
            var controllerTitle = new VideoController(repoTitle);
            var controllerDescription = new VideoController(repoDescription);

            // Act
            var resultTitle = controllerTitle.Search("Cory", false);
            var resultDescription = controllerDescription.Search("Cory", false);

            // Assert
            var okResultTitle = Assert.IsType<OkObjectResult>(resultTitle);
            var okResultDescription = Assert.IsType<OkObjectResult>(resultDescription);
            var actualVideosTitle = Assert.IsType<List<Video>>(okResultTitle.Value);
            var actualVideosDescription = Assert.IsType<List<Video>>(okResultDescription.Value);

            Assert.Equal(desiredResultCount, actualVideosTitle.Count);
            Assert.Equal(desiredResultCount, actualVideosDescription.Count);
            Assert.Equal(videosTitleTest.GetRange(0, desiredResultCount).OrderBy(p => p.DateCreated), actualVideosTitle);
            Assert.Equal(videosDescriptionTest.GetRange(0, desiredResultCount).OrderBy(p => p.DateCreated), actualVideosDescription);
        }
